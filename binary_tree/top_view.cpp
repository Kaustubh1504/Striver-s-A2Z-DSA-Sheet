...
class Solution
{
    ...
    //     if(node->left!=NULL) leftboundary(node->left,traversal);
    ...
    //     // if(node==NULL) return ;
    //     // traversal.push_back(node->data);
    //     // rightboundary(node->right,traversal);
    //     node=node->right;
    //      while(node!=NULL){
    //         traversal.push_back(node->data);
    ...
    //     }
    // }
    void levelorder(Node* node,vector<int>&topview){
